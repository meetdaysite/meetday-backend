...
// invoice this is always regenerated fresh — never trusted from a cached PDF. Mirrors
// SponsorshipReportPdfService's exact template, adapted for a Space deal's fields/names.
@Injectable()
export class SpaceReportPdfService {
  constructor(
  ...
  async getDownloadUrl(interestId: string): Promise<string> {
    ...
    const key = `space-deal-reports/${interestId}/report-${Date.now()}.pdf`;
  ...
  async generateForReport(interestId: string): Promise<Buffer> {
    const interest = await this.prisma.spaceInterest.findUnique({
      where: { id: interestId },
      include: {
        spaceCommunityProfile: { select: { name: true } },
    ...
    const deal = await this.prisma.spaceDeal.findUnique({ where: { spaceInterestId: interestId } });
    ...
    const requesterName =
      interest.requesterType === 'BRAND'
        ? (interest.brandProfile?.brandName ?? 'Brand')
        : (interest.hostProfile?.communityProfile?.name ?? interest.hostProfile?.displayName ?? 'Community');
    ...
    const projectName = parsed.projectName || report.projectName || deal.projectName || 'Project';
...
<body>
  ...
  <div class="overview-card">
    <table class="grid-table">
      <tr>
        <td class="label-cell">Space</td>
        <td class="val-cell">${escapeHtml(spaceName)}</td>
        <td class="label-cell">Event Date</td>
        <td class="val-cell">${escapeHtml(formattedEventDate || '—')}</td>
      </tr>
      <tr>
        <td class="label-cell">${interest.requesterType === 'BRAND' ? 'Brand' : 'Community'}</td>
        <td class="val-cell">${escapeHtml(requesterName)}</td>
  ...
  ${report.notes ? `
  <div class="section">
    <div class="section-header"><span class="section-dot"></span> Space Notes</div>
