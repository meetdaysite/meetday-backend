...
import { CommunityMemberStatus, DirectMessagePolicy, DmConversationStatus, DmMessageType } from '@prisma/client';
...
// `content` is stored encrypted-at-rest (AES-256-GCM); decrypted before returning.
...
/** Plaintext message payload from the client — the server encrypts text at rest. */
...
export class CommunityDmService {
  ...
  /** Decrypt the body and sign the image URL for an API/socket response. */
  private async present<T extends { content: string | null; mediaKey: string | null }>(
    message: T,
  ): Promise<T & { mediaUrl: string | null }> {
    const content = message.content ? this.crypto.decrypt(message.content) : null;
    const mediaUrl = message.mediaKey ? await this.storage.getPresignedDownloadUrl(message.mediaKey) : null;
    return { ...message, content, mediaUrl };
  }

  private previewOf(message: { content: string | null; messageType: DmMessageType } | undefined): string | null {
    if (!message) return null;
    if (message.messageType === DmMessageType.IMAGE && !message.content) return '📷 Photo';
    const text = message.content ? this.crypto.decrypt(message.content) : '';
  ...
  async createIntro(communityId: string, initiatorId: string, targetUserId: string, payload: DmMessagePayload) {
    ...
    return Promise.all(
      convos.map(async (c) => {
        ...
        return {
          conversationId: c.id,
          from,
          message: c.messages[0] ? await this.present(c.messages[0]) : null,
  ...
  async createMessage(conversationId: string, senderId: string, payload: DmMessagePayload) {
    const convo = await this.prisma.communityDmConversation.findUnique({
      where: { id: conversationId },
  ...
  async listConversations(communityId: string, userId: string) {
    const convos = await this.prisma.communityDmConversation.findMany({
      ...
      include: {
        ...
        messages: { orderBy: { createdAt: 'desc' }, take: 1, select: { content: true, messageType: true } },
      },
    });
    ...
    return Promise.all(
      convos.map(async (c) => {
        ...
        return {
          ...
          lastMessagePreview: this.previewOf(c.messages[0]),
          unreadCount,
        };
      }),
    );
  ...
  async getDmHistory(conversationId: string, userId: string, cursor?: string, limit = 30) {
    ...
    return { messages: await Promise.all(data.map((m) => this.present(m))), nextCursor };
  }
  ...
  private async insertMessage(conversationId: string, senderId: string, payload: DmMessagePayload) {
    if (!payload.content && !payload.mediaKey) {
      throw new BadRequestException('A message needs text or media');
    }
    const [message] = await this.prisma.$transaction([
      this.prisma.communityDmMessage.create({
        data: {
          ...
          messageType: payload.messageType ?? (payload.mediaKey ? DmMessageType.IMAGE : DmMessageType.TEXT),
